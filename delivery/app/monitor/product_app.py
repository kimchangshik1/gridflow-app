...
_MONITOR_MUTATION_METHODS = {"POST", "PUT", "PATCH", "DELETE"}
_MONITOR_ROUTE_PREFIX = "/monitor"
...
def _is_same_origin_monitor_mutation(request: Request) -> bool:
...
@app.middleware("http")
async def guard_monitor_mutations_same_origin(request: Request, call_next):
    if (
        request.method in _MONITOR_MUTATION_METHODS
        and request.url.path.startswith(_MONITOR_ROUTE_PREFIX)
        and not _is_same_origin_monitor_mutation(request)
    ):
        return JSONResponse(
            status_code=403,
