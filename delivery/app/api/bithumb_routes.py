...
_BITHUMB_RANKED_MAX_AGE_SEC = 1.5
_BITHUMB_RANKED_SOFT_REFRESH_SEC = 1.0
...
def _validate_bithumb_order_request(req: OrderRequest):
    side = req.side.upper()
    ...
    if side == "SELL":
        ...
        if req.qty is None and req.amount_krw <= 0:
            raise HTTPException(400, "매도 주문은 qty 또는 amount_krw가 필요합니다")
...
def get_ranked_symbols(request: Request, response: Response, user=Depends(get_current_user)):
    from app.core.cache import (
        ensure_bithumb_cache_refresh_async,
        get_bithumb_ticker_cache_with_state,
        refresh_bithumb_cache_blocking,
    )
    ...
    try:
        cache, ticker_cache_state = get_bithumb_ticker_cache_with_state(
            max_age_sec=_BITHUMB_RANKED_MAX_AGE_SEC,
            soft_age_sec=_BITHUMB_RANKED_SOFT_REFRESH_SEC,
        )
        refresh_state = "none"
        if ticker_cache_state == "warm":
            data = cache
        elif ticker_cache_state == "warming":
            data = cache
            started = ensure_bithumb_cache_refresh_async()
            ticker_cache_state = "warm"
            refresh_state = "started" if started else "inflight"
        elif ticker_cache_state == "warming_refreshing":
            data = cache
            ticker_cache_state = "warm"
            refresh_state = "inflight"
        elif ticker_cache_state == "empty":
            data = refresh_bithumb_cache_blocking()
            ticker_cache_state = "cold"
            refresh_state = "blocking"
        elif ticker_cache_state == "stale":
            data = cache
            started = ensure_bithumb_cache_refresh_async()
            ticker_cache_state = "stale"
            refresh_state = "started" if started else "inflight"
        else:
            data = cache
            ticker_cache_state = "stale"
        ...
        response.headers["X-Bithumb-Ticker-Refresh"] = refresh_state
        response.headers["X-Bithumb-Market-Name-Cache"] = market_name_cache_state
        return {"symbols": result}

    except Exception as e:
        print(f"[BITHUMB] 랭킹 심볼 조회 오류: {e}")
...
def create_order(req: OrderRequest, user=Depends(get_current_user)):
    _validate_bithumb_order_request(req)
    ...
    if user.get("is_dry_run"):
        ...
        try:
            ...
            print(f"[BITHUMB][SANDBOX] 주문 오류: {e}")
            raise HTTPException(500, "요청 처리 중 오류가 발생했습니다")
