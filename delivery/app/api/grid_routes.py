...
def _validate_grid_strategy_request(req: GridStrategyRequest):
    ...
    if req.profit_gap <= 0:
        raise HTTPException(400, "profit_gap은 0보다 커야 합니다")
    if req.max_investment is not None and req.max_investment <= 0:
        raise HTTPException(400, "max_investment는 0보다 커야 합니다")
    if req.stop_loss_price is not None and req.stop_loss_price <= 0:
        raise HTTPException(400, "stop_loss_price는 0보다 커야 합니다")
    if req.daily_loss_limit is not None and req.daily_loss_limit <= 0:
        raise HTTPException(400, "daily_loss_limit은 0보다 커야 합니다")
    if req.profit_target_pct is not None and req.profit_target_pct <= 0:
        raise HTTPException(400, "profit_target_pct는 0보다 커야 합니다")
    if req.smart_sell_mode not in ("BASIC", "SPLIT", "TRAILING", "BOTH"):
        raise HTTPException(400, "smart_sell_mode: BASIC / SPLIT / TRAILING / BOTH")
    if req.split_gap_pct <= 0:
        raise HTTPException(400, "split_gap_pct는 0보다 커야 합니다")
    if req.trailing_pct <= 0:
        raise HTTPException(400, "trailing_pct는 0보다 커야 합니다")
    if req.trailing_trigger_pct <= 0:
        raise HTTPException(400, "trailing_trigger_pct는 0보다 커야 합니다")
    if req.smart_sell_mode in ("SPLIT", "BOTH"):
        ...
        try:
            ...
            raise HTTPException(400, "split_ratio 형식이 올바르지 않습니다")
        if len(ratios) != req.split_count:
            raise HTTPException(400, f"split_ratio 항목 수는 split_count({req.split_count})와 같아야 합니다")
        if any(r <= 0 for r in ratios):
            raise HTTPException(400, "split_ratio 값은 모두 0보다 커야 합니다")
        if abs(sum(ratios) - 100) > 0.5:
            raise HTTPException(400, "split_ratio 합계는 100이어야 합니다")


@router.post("/strategies")
def create_strategy(req: GridStrategyRequest, user=Depends(get_current_user)):
    _validate_grid_strategy_request(req)
