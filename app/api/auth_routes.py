...
@router.get("/me")
def me(session: Optional[str] = Cookie(None), x_auth_token: Optional[str] = Header(None)):
    token = x_auth_token or session
    if not token:
        raise HTTPException(401, "로그인이 필요합니다")
    user = get_session(token)
...
@router.post("/users")
def add_user(req: CreateUserRequest, session: Optional[str] = Cookie(None), x_auth_token: Optional[str] = Header(None)):
    token = x_auth_token or session
...
def remove_user(user_id: int, session: Optional[str] = Cookie(None)):
    ...
    if not delete_user(user_id):
        raise HTTPException(400, "삭제 실패 (관리자 계정은 삭제 불가)")
    return {"success": True}
...
def toggle_dry_run(user_id: int, session: Optional[str] = Cookie(None), x_auth_token: Optional[str] = Header(None)):
