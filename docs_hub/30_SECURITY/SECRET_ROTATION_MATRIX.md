...
| --- | --- | --- | --- | --- |
| PostgreSQL credential | git history에 DB credential literal exposure가 있었다. 2026-04-25 UTC runtime rotation 후 current runtime password는 historically exposed literal과 더 이상 일치하지 않는다. gridflow-app, upbit-bot, orderlens-ops 재기동과 backup/restore verify까지 통과했다. | history exposure remains in full history, but current runtime reuse is closed | rotated confirmed | current runtime은 새 password로 재주입됐다. old literal을 buyer bundle이나 운영 기준값처럼 설명하지 않는다. full history repo는 계속 non-deliverable이다. |
