...
- captured:
  ...
  - `evidence/raw/monitor/monitor_authenticated_login_flow.txt` — 임시 로컬 runtime에서 `GET /monitor/auth-status -> POST /monitor/login -> GET /monitor -> GET /monitor/orders -> GET /monitor/activity`가 모두 200으로 이어진 sanitized flow
  - `evidence/raw/monitor/monitor_authenticated_home_runtime.png` — 같은 authenticated runtime의 `/monitor` home safe crop
  - `evidence/raw/monitor/monitor_authenticated_orders_activity_runtime.png` — 같은 authenticated runtime에서 recent orders/activity/filter가 보이는 safe crop, 본문 행은 마스킹
- provenance:
  - 이번 raw pack의 홈 화면 캡처는 배포 중인 `monitor.html` 자산 기준이다. `app/monitor/product_app.py`의 `/monitor`는 인증 후 동일 자산을 반환한다.
  - authenticated runtime pack은 같은 monitor 앱 코드를 임시 로컬 runtime으로 띄워, 격리된 `monitor_auth/config`로 실제 로그인 후 `/monitor`와 `/monitor/orders`, `/monitor/activity` 응답을 확인한 뒤 캡처했다.
  - 이번 pack은 buyer-facing safe capture를 위한 것이며, live operator 세션 provenance, restart 후 session persistence, multi-instance coherence를 증명하지 않는다.
...
- monitor authenticated post-login runtime proof: confirmed
...
- missing: 배포 중 monitor에서 live operator credential로 진입한 authenticated runtime provenance
- missing: restart 후 monitor session persistence direct proof
- missing: multi-instance monitor session coherence direct proof
