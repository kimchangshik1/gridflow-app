...
async function authFetch(url, options = {}) {
  if (window.isGuest) {
    ...
    if (_gu.indexOf('/orders') > -1 || _gu.indexOf('/positions') > -1) {
      var _oh = Object.assign({}, options.headers || {}, { 'X-Auth-Token': window._token });
      return fetch(url, Object.assign({}, options, { headers: _oh }));
    }
    if (_gu.indexOf('/grid') > -1 || (_gu.indexOf('/bapi/orders') > -1)) {
      var _gm = (options && options.method) ? options.method.toUpperCase() : 'GET';
      if (_gm === 'POST') {
        var _gh2 = Object.assign({}, options.headers || {}, { 'X-Auth-Token': window._token });
        return fetch(url, Object.assign({}, options, { headers: _gh2 }));
      }
  ...
  const headers = { ...(options.headers || {}), 'X-Auth-Token': _token }
...
async function init() {
  ...
  if (!window._currentUser && typeof _token !== 'undefined' && _token && typeof checkAuth === 'function') {
    try {
      await checkAuth();
    } catch (e) {}
  }
