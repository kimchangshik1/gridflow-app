...
window.__homeRuntimeIsMobileLayout = function() {
  if (typeof window === 'undefined') return false;
  if (window.matchMedia) return window.matchMedia('(max-width: 768px)').matches;
  return window.innerWidth <= 768;
};

window.__homeOpenStrategyDetail = function(type, id) {
  var normalizedType = String(type || '').toLowerCase();
  var strategyId = id == null ? '' : String(id);
  if (normalizedType !== 'dca' && normalizedType !== 'rebal') normalizedType = 'grid';
  if (!strategyId) return Promise.resolve(false);
  if (typeof window.openStrategyDeepLink === 'function') return window.openStrategyDeepLink(normalizedType, strategyId, 'home');
  if (typeof switchGrid === 'function') switchGrid();
  if (typeof switchGridTab === 'function') switchGridTab(normalizedType);
  return Promise.resolve(false);
};

window.__homeApplyBannerStrategyTarget = function(target) {
  var bannerLink = document.getElementById('gsb-link');
  if (!bannerLink) return;

  var strategyType = target && target.type != null ? String(target.type).toLowerCase() : '';
  var strategyId = target && target.id != null ? String(target.id).trim() : '';
  var hasTarget = !!strategyId && (strategyType === 'grid' || strategyType === 'dca' || strategyType === 'rebal');
  ...
  if (!hasTarget) {
    ...
    bannerLink.removeAttribute('data-strategy-type');
    bannerLink.removeAttribute('data-strategy-id');
  ...
  bannerLink.onclick = function(e) {
    e.preventDefault();
    window.__homeOpenStrategyDetail(strategyType, strategyId);
    return false;
  };
  bannerLink.dataset.strategyType = strategyType;
  bannerLink.dataset.strategyId = strategyId;
  bannerLink.removeAttribute('aria-hidden');
};

window.__homeApplyBannerStrategyTarget(null);

window.__homeResolveBannerStrategyTarget = function(gridData, dcaData, rebalData) {
  var groups = [
    { type: 'grid', strategies: gridData && gridData.strategies ? gridData.strategies : [] },
    { type: 'dca', strategies: dcaData && dcaData.strategies ? dcaData.strategies : [] },
    { type: 'rebal', strategies: rebalData && rebalData.strategies ? rebalData.strategies : [] }
  ];

  function firstMatch(statuses) {
    for (var i = 0; i < groups.length; i += 1) {
      var strategies = groups[i].strategies;
      for (var j = 0; j < strategies.length; j += 1) {
        var strategy = strategies[j];
        if (statuses.indexOf(strategy.status) !== -1) {
          return { type: groups[i].type, id: strategy.id };
        }
      }
    }
    return null;
  ...
  var errorTarget = firstMatch(['ERROR']);
...
window.renderDashStatus = function(gridData, dcaData, rebalData) {
  ...
  /* 글로벌 배너 */
  var bannerTarget = window.__homeResolveBannerStrategyTarget(gridData, dcaData, rebalData);
  if (typeof updateGlobalBanner === 'function') {
    updateGlobalBanner(bannerStatus, bannerLabel, bannerMsg);
  }
  window.__homeApplyBannerStrategyTarget(bannerTarget);
...
window.renderHomePosTable = function(posData) {
  ...
  function fmtKrw(n){ return Math.floor(Number(n||0)).toLocaleString('ko-KR') + '원'; }
  function mobileStat(label, value) {
    return '<div class="hpp-mobile-stat">' +
      '<span class="hpp-mobile-stat-label">' + label + '</span>' +
      '<span class="hpp-mobile-stat-val">' + value + '</span>' +
    '</div>';
  }
  ...
  el.innerHTML = sorted.map(function(p) {
    ...
    if (isMobile) {
      return '<div class="hpp-mobile-card">' +
        '<div class="hpp-mobile-head">' +
          '<div class="hpp-mobile-asset">' +
            '<span class="hpp-mobile-symbol">' + sym + '</span>' +
            '<span class="hpp-mobile-name">' + kname + '</span>' +
          '</div>' +
          '<span class="hpp-mobile-pct" style="' + pctColor + '">' + pctText + '</span>' +
        '</div>' +
        '<div class="hpp-mobile-exchange">' + exName + '</div>' +
        '<div class="hpp-mobile-stats">' +
          mobileStat('평가금액', fmtKrw(evalAmount)) +
          mobileStat('수량', fmtQty(p.qty)) +
        '</div>' +
        '<div class="hpp-mobile-foot">' +
          '<div class="hpp-mobile-pnl-wrap">' +
            '<span class="hpp-mobile-pnl-label">미실현 손익</span>' +
            '<span class="hpp-mobile-pnl" style="' + pctColor + '">' + fmtPnl(pnl) + '</span>' +
          '</div>' +
          '<button class="hpp-mobile-detail-btn" onclick="' + detailAction + '">포지션 상세</button>' +
        '</div>' +
      '</div>';
    }
    return '<div class="hpp-row hpp-cols">' +
      ...
      '<div class="hpp-cell r">' + fmtKrw(evalAmount) + '</div>' +
      '<div class="hpp-cell r" style="'+pctColor+'">' + pctText + '</div>' +
...
window.renderDashTop = function(balData, posData, gridData, dcaData, rebalData) {
  ...
  if (posEl) posEl.textContent = visiblePositions.length + '종목';
...
window.__homeRuntimeRenderStrategyList = function(gridData, dcaData, rebalData) {
  ...
  setT('hsp-count-summary', '실행 ' + running + ' · 일시정지 ' + paused + ' · 대기 ' + pending);
  ...
  function escapeAttr(val) { return String(val == null ? '' : val).replace(/&/g, '&amp;').replace(/"/g, '&quot;'); }
  function mobileBtn(label, action, cls) {
    return '<button class="hsp-mobile-btn ' + cls + '" onclick="event.stopPropagation();' + action + '">' + label + '</button>';
  }
  ...
  el.innerHTML = all.map(function(s) {
    ...
    if (s.status === 'ACTIVE' && (s._type === 'grid' || s._type === 'dca')) {
      ...
      mainBtn = isMobile
        ? mobileBtn('일시정지', fn + '(' + s.id + ')', 'warn')
        : '<button onclick="event.stopPropagation();' + fn + '(' + s.id + ')" style="padding:3px 8px;font-size:10px;font-weight:700;border-radius:4px;cursor:pointer;border:1px solid var(--color-warning);color:var(--color-warning);background:rgba(245,158,11,0.1)">⏸ 정지</button>';
    } else if (s.status === 'PAUSED') {
      var fn2 = s._type === 'grid' ? 'resumeGrid' : s._type === 'dca' ? 'resumeDCA' : '';
      if (fn2) {
        mainBtn = isMobile
          ? mobileBtn('재개', fn2 + '(' + s.id + ')', 'ok')
          : '<button onclick="event.stopPropagation();' + fn2 + '(' + s.id + ')" style="padding:3px 8px;font-size:10px;font-weight:700;border-radius:4px;cursor:pointer;border:1px solid var(--color-normal);color:var(--color-normal);background:rgba(16,185,129,0.1)">▶ 재개</button>';
      }
    } else if (s.status === 'ERROR') {
      mainBtn = isMobile
        ? mobileBtn('오류 확인', 'switchGrid()', 'danger')
        : '<button onclick="event.stopPropagation();switchGrid()" style="padding:3px 8px;font-size:10px;font-weight:700;border-radius:4px;cursor:pointer;border:1px solid var(--color-error);color:var(--color-error);background:rgba(239,68,68,0.1)">⚠ 확인</button>';
    }
    var detailAction = "__homeOpenStrategyDetail('" + s._type + "'," + s.id + ")";
    var cardLabel = typeLabel(s._type) + ' 전략 ' + name + ' 상세 보기';
    var escapedName = escapeAttr(name);
    var escapedCardLabel = escapeAttr(cardLabel);
    var cardAttrs =
      ' role="button"' +
      ' tabindex="0"' +
      ' data-strategy-type="' + s._type + '"' +
      ' data-strategy-id="' + s.id + '"' +
      ' aria-label="' + escapedCardLabel + '"' +
      ' onclick="' + detailAction + '"' +
      ' onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();' + detailAction + '}"' +
      ' onfocus="this.style.outline=\'2px solid rgba(245,158,11,0.9)\';this.style.outlineOffset=\'2px\'"' +
      ' onblur="this.style.outline=\'none\';this.style.outlineOffset=\'0\'"' +
      ' style="cursor:pointer"';
    var detailBtn = isMobile
      ? mobileBtn('상세 보기', detailAction, 'neutral')
      : '<button onclick="event.stopPropagation();' + detailAction + '" style="padding:3px 8px;font-size:10px;font-weight:700;border-radius:4px;cursor:pointer;border:1px solid rgba(255,255,255,0.1);color:var(--text3);background:rgba(255,255,255,0.05)">상세</button>';

    if (isMobile) {
      return '<div class="hsp-mobile-card"' + cardAttrs + '>' +
        '<div class="hsp-mobile-head">' +
          '<span class="hsp-mobile-type hsp-mobile-type-' + s._type + '">' + typeLabel(s._type) + '</span>' +
          '<div class="hsp-mobile-name" title="' + escapedName + '">' + name + '</div>' +
        '</div>' +
        '<div class="hsp-mobile-market">' + exName + ' · ' + sym + '</div>' +
        '<div class="hsp-mobile-status-row">' +
          '<span class="status-badge ' + stCls + '">' + stTxt + '</span>' +
        '</div>' +
        '<div class="hsp-mobile-stats">' +
          '<div class="hsp-mobile-stat">' +
            '<span class="hsp-mobile-stat-label">마지막 실행</span>' +
            '<span class="hsp-mobile-stat-val">' + lastRun + '</span>' +
          '</div>' +
          '<div class="hsp-mobile-stat">' +
            '<span class="hsp-mobile-stat-label">누적 손익</span>' +
            '<span class="hsp-mobile-stat-val" style="' + pnlStyle + '">' + fmtPnl(pnl) + '</span>' +
          '</div>' +
        '</div>' +
        '<div class="hsp-mobile-actions">' +
          (mainBtn || '') + detailBtn +
        '</div>' +
      '</div>';
    }

    return '<div class="hsp-row"' + cardAttrs + '>' +
      '<div class="hsp-cell name" title="' + escapedName + '">' + name + '</div>' +
