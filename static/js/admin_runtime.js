...
window.__adminRuntimeFetchUsers = async function() {
  ...
  tbody.innerHTML = users.map(u => {
    ...
    const adminBadge = u.is_admin
      ? '<span style="font-size:11px;font-weight:700;color:#F59E0B;background:rgba(245,158,11,0.12);padding:2px 7px;border-radius:5px;border:1px solid rgba(245,158,11,0.25)">👑 관리자</span>'
      : '<span style="font-size:11px;color:#6B7280">일반</span>';
    const isSelf = u.id === _currentUser.user_id;
    const actionBtns = isSelf
      ? '<span style="font-size:11px;color:#4B5563;font-style:italic">본인</span>'
    ...
    return `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);height:42px">
      ...
      <td style="padding:0 12px">${adminBadge}</td>
