...
현재 backend control plane으로 설명 가능한 경로:

- Grid pause / resume
- DCA pause / resume
- Rebalancing pause / resume

다만 emergency stop/release는 아래처럼 구분해야 합니다.

- verified by integration tests: Grid/DCA pause/resume backend control path
- documented by evidence: 일부 runtime emergency stop safeguard
- not a documented backend contract: dedicated emergency release endpoint

즉, 현재 제품 문서에는 “전용 backend emergency release API”를 포함시키지 않습니다.
